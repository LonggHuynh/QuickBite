...
export async function createOrder(order: AppOrder): Promise<AppOrder> {
  ...
  if (dishes.length === 0) {
    throw new CustomError(400, 'No dishes found for the given restaurant');
  }
  ...
  if (amount_paid !== total) {
    throw new CustomError(400, 'There is something wrong with your order.');
  }

  if (total - restaurantDetails.delivery_fee < restaurantDetails.min_order) {
    throw new CustomError(400, 'The minimum order price has not been reached yet.');
  }
