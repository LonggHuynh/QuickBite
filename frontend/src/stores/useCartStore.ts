...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  addToCart: (dish: Dish, restaurant?: Restaurant) =>
    set((state: CartState) => {
  ...
  calculateTotal: () => {
    const state = get();
    return (
      state.calculateSubtotal() + Number(state.restaurant?.delivery_fee ?? 0)
    );
  },
  ...
  okToCheckout: () => {
    ...
    return (
      ...
      (state.restaurant?.min_order ?? 0) <= state.calculateTotal()
    );
  },
}));
