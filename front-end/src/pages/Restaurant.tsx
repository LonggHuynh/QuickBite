...
const Restaurant = () => {
    ...
    const { data: restaurant, isLoading: isRestaurantLoading } = useGetRestaurantById(id!);
    const { data: dishes, isLoading: isDishesLoading } = useGetDishesByRestaurantId(id!);
    const categorizedDishes = dishes
        ? Object.values(
            dishes.reduce((acc: { [key: string]: typeof dishes }, dish) => {
    ...
    return restaurant ? (
        <>
            ...
            {addDish && (
                <EditDish cardType={CardType.Create} closeTab={() => setAddDish(false)} categories={[]} restaurant_id={restaurant.id} />
            )}
            <div className="flex">
